(function plantaoIntegrityUiFix() {
    ...
    function installSafeTaskCard() {
        ...
        function safeTaskCard(task, day, index, overdue) {
            const data = state();
            const realIndex = overdue && Array.isArray(data?.metaFixa?.[day])
                ? Math.max(0, data.metaFixa[day].indexOf(task))
                : index;
        ...
        const adminPanel = admin ? `
            <div class="stat-card admin-access-card">
                <div class="admin-access-head">
                    <div><h3>Alunos</h3><p class="meta-sub">Gerencie acessos e planejamentos sem exibir dados de login.</p></div>
                    <button class="btn btn-sm btn-outline" onclick="carregarSolicitacoesAcesso()"><i class="fas fa-rotate"></i> ATUALIZAR</button>
                </div>
        ...
        target.innerHTML = `
            <div class="profile-grid">
                <div class="stat-card profile-main-card">
                    <div class="profile-avatar">${avatar ? `<img src="${esc(avatar)}" alt="Foto de perfil">` : `<span>${esc(initials)}</span>`}</div>
                    <div><small>Conta conectada</small><h3>${nameHtml}</h3><p>${esc(detail)}</p></div>
                </div>
                <div class="stat-card profile-info-card">
                    <div class="profile-info-row"><span>Nome</span><strong>${nameHtml}</strong></div>
                    <div class="profile-info-row"><span>Perfil</span><strong>${esc(fn('loginUsuario')?.() || 'Aluno')}</strong></div>
                    <div class="profile-info-row"><span>Status</span><strong>${esc(status)}</strong></div>
                </div>
                <div class="stat-card profile-actions-card">
                    <h3>Nome na plataforma</h3>
                    <p class="meta-sub">Este nome aparece nas telas de acompanhamento.</p>
                    <label for="perfil-nome-aluno">Nome exibido</label>
                    <input type="text" id="perfil-nome-aluno" value="${nameHtml}" maxlength="80" placeholder="Nome do aluno">
                    <button class="btn" onclick="salvarNomePerfil()"><i class="fas fa-user-check"></i> SALVAR NOME</button>
                </div>
                <div class="stat-card profile-actions-card">
                    <h3>Acesso</h3>
                    <p class="meta-sub">Saia desta conta para entrar com outro usuario neste dispositivo.</p>
                    <button class="btn btn-outline" onclick="sairGoogle()"><i class="fas fa-right-from-bracket"></i> SAIR DO LOGIN</button>
                </div>
                <div class="stat-card profile-deadline-card">
                    <h3>Data do edital</h3>
                    <label for="edital-publicacao">Publicacao do edital</label>
                    <input type="date" id="edital-publicacao" value="${esc(data.editalPublicacao || '')}">
                    <button class="btn" onclick="salvarDataEdital()"><i class="fas fa-calendar-check"></i> SALVAR DATA</button>
                </div>
                ${adminPanel}
            </div>`;
        ...
        function safeList() {
            ...
            target.innerHTML = students.map(item => {
                ...
                const actions = item.status === 'approved'
                    ? `<button class="btn btn-sm btn-outline" onclick="entrarPerfilAluno('${key}')">EDITAR PERFIL</button><button class="btn btn-sm btn-outline danger-btn" onclick="alterarAcessoAluno('${key}', 'rejected')">REVOGAR</button>`
                    : `<button class="btn btn-sm" onclick="alterarAcessoAluno('${key}', 'approved')">APROVAR</button><button class="btn btn-sm btn-outline danger-btn" onclick="alterarAcessoAluno('${key}', 'rejected')">RECUSAR</button>`;
    ...
    function install() {
        ...
        [
            'normalizarBanco', 'renderTree', 'renderReverSinalizados', 'renderFluxo', 'renderCiclo',
            'renderDiario', 'renderSemanal', 'renderLancamentos', 'renderPerformance',
            'renderRankingAlunos', 'renderPainelProfessor', 'renderEvolucaoEstudo'
        ].forEach(patchBeforeRender);
